...
def sample_pre_process(Xarr,Y_full,bins,mean_dp,var_dp,num_chunks,seed,pheno):
    ...
    downsampled_chunks = downsample_chunks(chunks, 3000, rng)
    K = computeGRM(Xarr)
    ...
    for i in range(num_chunks):
        chunk_i = downsampled_chunks[i]
        mean_i = mean_dp[chunk_i]
        A_temp = []

        for j in range(num_chunks):
            chunk_j = downsampled_chunks[j]
            mean_j = mean_dp[chunk_j]
            ixgrid = np.ix_(chunk_i, chunk_j)
            di = diag[chunk_i]  # shape: (ni,)
            dj = diag[chunk_j]  # shape: (nj,)
            ...
            if i == j:
                ...
                A_temp.append(compute_A_for_chunk(K[ixgrid], Ycorr, diff_yy, Y_uniq, var_dp,mean_i, mean_j,len(chunks[i]) ,len(chunks[j]),zero_diag=True))
            else:
                # off-diagonal chunk, zero_diag=False
                A_temp.append(compute_A_for_chunk(K[ixgrid], Ycorr, diff_yy, Y_uniq, var_dp,mean_i, mean_j, len(chunks[i]),len(chunks[j]), zero_diag=False))
...
def downsample_chunks(chunks, n_samples_per_chunk, rng):
    downsampled_chunks = []
    ...
    for chunk in chunks:
        if len(chunk) <= n_samples_per_chunk:
            ...
            downsampled_chunks.append(rng.choice(chunk, size=n_samples_per_chunk, replace=False))
    
    return downsampled_chunks
...
def compute_A_for_chunk(K,Ycorr,d_yy,Y_uniq,var,mean_i,mean_j,full_ni,full_nj,zero_diag):
    ...
    # Scale C to match what it would be if using the full chunk sizes
    if zero_diag:
        # Adjust for exclusion of self-pairs
        scaling = (full_ni * (full_ni - 1)) / (ni * (ni - 1)) if ni > 1 else 1.0
    else:
        scaling = (full_ni * full_nj) / (ni * nj)
...
def compute_partial_C_for_row(args):
    ...
    if zero_diag:
        if i < len(K_row):
            K_row = K_row.copy()  # avoid modifying original array
            K_row[i] = 0.0 
...
def opt_dca(A,B,E,sz_y,sz_yhat,eps,max_iter,optTol,num_chunks):
    ...
    # print(np.min(B),np.max(B),flush=True)
    ...
    for itr in range(max_iter):
        ...
        print(f"Solution: Status - {sol['status']}, Primal obj value- {sol['primal objective']}")
        print(f"Function minimum: {fval}")
...
def joint_prob_dist(corr,U,var):
    
#     num = np.square(u)/var_x + np.square(v)/var_y - ((2*corr)*u*v/(np.sqrt(var_x*var_y)))
    denom = 2*math.pi*var*((1-corr**2))**.5
    num = U + U.T - ((2*corr)*np.sqrt(U*U.T))
    num = np.exp(-num /(2*var*(1-corr**2)))
    return num /denom  #/np.sum(num) 
