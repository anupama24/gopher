...
#     pheno_df['group'] = pd.qcut(pheno_df[pheno_name].rank(method='first'), q=50, labels=False) + 1
...
#     group_max = Y_full.max()
#     group_min = Y_full.min()
...
#     overall_var_dp = np.var(Y) + laplace_noise(sensitivity, eps1*0.9, 1)
...
#         pheno_df[['IID', 'dp_group_mean']],
#         on='IID',
#         how='left'
#     )

#     score_df['mean_score'] = score_df['SCORE1_SUM'] * overall_var_dp + score_df['dp_group_mean']
#     mean_dp = score_df['mean_score'].values
    ...
    Q1,Q2,B,Y_uniq,Y_hat,chunks = sample_pre_process(X_snp,Y,bins,mean_dp,y_var,num_chunks,seed)
