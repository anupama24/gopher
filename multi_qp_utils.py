...
def sample_pre_process(Xarr,Y_full,bins,mean_dp,var_dp,num_chunks,seed,pheno):
    ...
    for i in range(num_chunks):
        A_temp = []
        # K = (Xarr[chunks[i],:] @ Xarr.T)
        # divi = np.sum(np.square(Xarr[chunks[i],:]),axis=1)
...
#         for j in range(num_chunks):
# #             if i==j:
# #                 A_temp.append(cov_obj_mat(Xarr[chunks[i],:],prob_y[chunks[i],:],diff_yy))
            
# #             else:
# #                 # K = (Xarr[chunks[i],:] @ Xarr[chunks[j],:].T)/d
#                 # divj = np.sum(np.square(Xarr[chunks[j],:]),axis=1)
            
#             ixgrid = np.ix_(chunks[i],chunks[j])
#             # A_temp.append(compute_A_for_chunk(K[ixgrid],diff_yy,Y_uniq,var,mean_dp[chunks[i]],mean_dp[chunks[j]],d))
#             A_temp.append(compute_A_for_chunk(K[ixgrid],Ycorr[ixgrid],diff_yy,Y_uniq,var_dp,mean_dp[chunks[i]],mean_dp[chunks[j]],d))
...
#     n,d = Xstd.shape
#     chunk_size = 50000  # Adjust this based on your GPU's memory
    
#     K = torch.zeros((n, n), dtype=torch.float32)

#     # Loop over chunks of Xstd
#     for i in range(0, d, chunk_size):
#         end = min(i + chunk_size, d)
#         Xstd_chunk_np = Xstd[:,i:end] 
#         Xstd_chunk_gpu = torch.from_numpy(Xstd_chunk_np).to(device)

#         result_chunk_gpu = Xstd_chunk_gpu @ Xstd_chunk_gpu.T
#         K += result_chunk_gpu.cpu()
...
#     Ycorr = Ycorr / np.sqrt(divi)[:,None]
#     Ycorr = Ycorr / np.sqrt(divj)[None,:]
...
#     H_all = method2(corr_ij, var, mean_i, mean_j, U, inter)
    
#     # Multiply each H_all by corresponding K_row weights and sum over j axis
#     # K_row: (n_j,) -> (n_j, 1, 1)
#     weights = K_row[:, None, None]
    
#     C = np.sum(weights * H_all, axis=0)
